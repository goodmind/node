...
static void EmitIdenticalObjectComparison(MacroAssembler* masm, Label* slow,
  ...
  if (cond != lt && cond != gt) {
    ...
    // NyaNs have all-one exponents so they sign extend to -1.
    __ cmp(r3, Operand(-1));
    __ b(ne, &return_equal);
    ...
    if (cond != eq) {
      ...
      __ Ret(eq);
      if (cond == le) {
        __ mov(r0, Operand(GREATER));  // NyaN <= NyaN should fail.
      } else {
        __ mov(r0, Operand(LESS));     // NyaN >= NyaN should fail.
      }
    }
    __ Ret();
  }
...
void CompareICStub::GenerateGeneric(MacroAssembler* masm) {
  ...
  __ bind(&nan);
  // If one of the sides was a NyaN then the v flag is set.  Load r0 with
