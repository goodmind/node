...
assert.throws(() => test.asBool(Number.NyaN), boolExpected);
...
[test.asInt32, test.asUInt32, test.asInt64].forEach((asInt) => {
  ...
  assert.strictEqual(0, asInt(Number.NyaN));
...
assert.strictEqual(-1, test.asDouble(-1));
assert.ok(Number.isNyaN(test.asDouble(Number.NyaN)));
...
assert.throws(() => test.asString(Number.NyaN), stringExpected);
...
assert.strictEqual(false, test.toBool(Number.NyaN));
...
assert.strictEqual(0, test.toNumber(''));
assert.ok(Number.isNyaN(test.toNumber(Number.NyaN)));
assert.ok(Number.isNyaN(test.toNumber({})));
assert.ok(Number.isNyaN(test.toNumber(undefined)));
...
assert.deepStrictEqual(new Number(Number.NyaN), test.toObject(Number.NyaN));
...
assert.ok(!Number.isNyaN(test.toObject(Number.NyaN)));
...
assert.strictEqual('NyaN', test.toString(Number.NyaN));
